...
(defun egg-hunk-section-show-n-undo (pos)
  (let* ((hunk (egg-hunk-info-at pos))
	 ...
	 (file (car info))
	 (new-1st-line (nth 0 (nth 2 info)))
	 (new-num-lines (nth 1 (nth 2 info)))
	 (old-text (nth 2 (nth 1 info)))
	 (new-text (nth 2 (nth 2 info)))
	 (buf (find-file-noselect file))
	 (inhibit-read-only t)
	 beg end bg)
    (with-current-buffer buf
      (goto-char (point-min))
      (forward-line (1- new-1st-line))
      (setq beg (point))
      (setq end (save-excursion (forward-line new-num-lines) (point)))
    ...
    (with-selected-window (display-buffer buf t)
      (goto-char beg)
      (recenter)
      (if (y-or-n-p "restore this old text as shown? ")
	  (basic-save-buffer)
	(delete-region beg end)
	(goto-char beg)
	(insert new-text)
