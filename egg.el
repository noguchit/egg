...
(defun egg-sync-do-stdin (program accepted-codes input args)
  (let (output ret)
...
(defun egg-sync-do-file-stdin (file program accepted-codes input args)
  (let ((default-directory (file-name-directory (egg-git-dir))))
    (setq file (expand-file-name file))
    (setq args (mapcar (lambda (word)
			 (if (string= word file) file word))
		       args))
    (if (egg-sync-do-stdin program accepted-codes input args)
	file)))

(defsubst egg-sync-git-file-stdin (file input args)
  (egg-sync-do-file-stdin file "git" nil input args))

(defsubst egg-sync-git-file-stdin-1 (file input args)
  (egg-sync-do-file-stdin file "git" 1 input args))

(defun egg-hunk-section-patch-cmd (pos &rest args)
  (let ((patch (egg-hunk-section-patch-string pos))
	(file (car (get-text-property pos :diff))))
    (unless (stringp file)
      (error "No diff with file-name here!"))
    (egg-sync-git-file-stdin file patch args)))

(defun egg-hunk-section-cmd-stage (pos)
  (interactive (list (point)))
  (egg-hunk-section-patch-cmd pos "apply" "--cached"))

(defun egg-hunk-section-cmd-unstage (pos)
  (interactive (list (point)))
  (egg-hunk-section-patch-cmd pos "apply" "--cached" "--reverse"))
...
(defun egg-hunk-section-cmd-undo (pos)
  ...
  (let ((patch (egg-hunk-section-patch-string pos))
    ...
    (unless (stringp file)
      (error "No diff with file-name here!"))
    (setq file-full-name
	  (egg-sync-do-file-stdin file "patch" nil patch
				  '("-p1" "--quiet" "--reverse")))
